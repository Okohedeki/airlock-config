...
Airlock Config is a config format and open-source tooling for declaring how an agent-driven service handles inbound traffic from external (often untrusted) agents. The file is the **config that publishes your business agent's externally observable interface** so other businesses' agents can discover and integrate with it. This document defines the language used throughout the project so terms mean the same thing in code, in docs, and in conversation.

## Language

**Contract**:
A self-declared, machine-readable specification published by a receiving party describing what inbound agent-driven interactions it accepts, refuses, and escalates. Authored by the receiver; consumed by the sender. NOT a bilateral agreement — neutrality comes from the open schema, not from co-authorship. Analogue: OpenAPI. Throughout this codebase "contract" and "Airlock Config" refer to the same artifact; the file extension is `.airlock-config.yaml` and the top-level YAML key is `airlock_config:`.
...
**A2A — Agent to Agent**:
Two agents communicate directly. Airlock Config standardizes the receiving side's expectations. Typically synchronous and fast.
...
The publisher is bound. If the real agent diverges from a PROMISE pre-flight verdict, that is a public conformance violation catchable by `airlock-config check`. Produced by deterministic rules (phases 1–3 of the taxonomy).
...
**Authority rule**:
A declarative rule in the contract that determines what the agent will do unilaterally vs. refuse vs. escalate. Each rule declares itself `deterministic` (→ PROMISE codes) or `judgment` (→ ESTIMATE codes). Targets a `skill`.
...
The A2A v1.0 manifest a publisher serves at `/.well-known/agent-card.json` so A2A-speaking consumers can discover the agent without any Airlock-Config-specific code. Always derived from the Airlock Config contract by `airlock-config build-site` — publishers never hand-author both files. The Agent Card carries name, description, endpoint, skill list, and security schemes; the Airlock Config contract carries everything else (category, region, compliance, pricing, authority rules with PROMISE/ESTIMATE bindings, etc). Cross-referenced via `extensions[].uri = "airlock-config-contract"`. See ADR 0007 and `docs/a2a-bridge.md`.
_Avoid_: A2A manifest, A2A profile, agent.json (the older path — A2A v1.0 uses agent-card.json)

**A2A wire** (the protocol layer):
JSON-RPC 2.0 over HTTP + Server-Sent Events. The sandbox accepts `POST /a2a` and implements three methods: `SendMessage`, `GetTask`, `CancelTask`. Streaming and push notifications are deferred. A2A is the wire; Airlock Config is the capability disclosure on top of it.
...
The canonical location of a published contract: `https://<host>/.well-known/airlock-config.yaml` for the machine spec, `/.well-known/airlock-config/` for the rendered human + LLM docs. RFC 8615 pattern. (v0.4 served at `/.well-known/airlock.yaml`; see `docs/migration-v04-to-v05.md`.)
...
**"Gateway"** is *not* an Airlock Config term. The doc and conversation have used it loosely. Airlock Config is not a runtime gateway; it never holds traffic. If "gateway" appears in code or docs, replace it with the precise term (e.g., "channel adapter," "static bundle," "self-hosted runtime" if referring to the future sister project's role).
...
> **Domain expert:** You're the **publisher**. Write a **contract** describing what your service accepts. Drop it at your **well-known contract URL** (`/.well-known/airlock-config.yaml`). Your customers' agents — the **consumers** — discover it from there.
>
...
> **Domain expert:** PROMISE comes from your **deterministic** rules — the math holds. ESTIMATE comes from your **judgment** rules — depends on model output or inventory or human review. `airlock-config check` will publicly fail your contract if you ever lie on a PROMISE.
>
