...
describe("buildAgentCard — derivation from supplier-agent", () => {
  it("derives the top-level identity fields", () => {
    ...
    const card = buildAgentCard(contract, {
      contractUrl: "https://example.com/.well-known/airlock.yaml",
    });
  ...
  it("includes an airlock-contract back-pointer extension", () => {
    const contract = loadSupplier();
    const card = buildAgentCard(contract, {
      contractUrl: "https://example.com/.well-known/airlock.yaml",
    });
    expect(card.extensions).toEqual([
      { uri: "airlock-contract", value: "https://example.com/.well-known/airlock.yaml" },
    ]);
  });

  it("leaves signature undefined in v0.4.1 (Stage 3 deferred to v0.5)", () => {
    const contract = loadSupplier();
    const card = buildAgentCard(contract, {
      contractUrl: "https://example.com/.well-known/airlock.yaml",
    });
...
describe("buildAgentCard — defaults when a2a block is absent", () => {
  function minimal(): AirlockContract {
    return {
      airlock: "0.4.1",
  ...
  it("derives endpoint URL from the contract URL host", () => {
    const card = buildAgentCard(minimal(), {
      contractUrl: "https://hello.example.com/.well-known/airlock.yaml",
    });
