...
function loadProcurement(): AirlockContract {
  const result = validateContractFile(PROCUREMENT);
  if (!result.ok || !result.contract) {
    throw new Error(`procurement example is not valid: ${JSON.stringify(result.issues, null, 2)}`);
  }
...
describe("pipeline — happy path", () => {
  it("ACCEPTED_BY_RULE for a small date change (PROMISE)", () => {
    const contract = loadProcurement();
