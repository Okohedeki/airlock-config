...
// executes the bundle in Node, confirming window.airlockConfig.evaluate(...)
// matches the Node sandbox.
import { readFileSync } from "node:fs";

// Site layout puts each example under examples/<agent-name>/
const PAGE = "dist-pages/examples/acme-supplier-agent/.well-known/airlock-config/index.html";

const html = readFileSync(PAGE, "utf-8");

const contractMatch = html.match(
  /<script>window\.__AIRLOCK_CONFIG_CONTRACT__ = (.+?);<\/script>/,
);
...
globalThis.__AIRLOCK_CONFIG_CONTRACT__ = JSON.parse(contractMatch[1]);

new Function(bundleMatch[1])();

if (!globalThis.airlockConfig) throw new Error("window.airlockConfig was not set by the bundle");
...
for (const s of scenarios) {
  const result = globalThis.airlockConfig.evaluate(s.skill, s.input, "preflight");
...
const a = globalThis.airlockConfig.evaluate("query_inventory", { sku: "SKU-42" }, "skills");
const b = globalThis.airlockConfig.evaluate("query_inventory", { sku: "SKU-42" }, "skills");
